...
[[nodiscard]] inline size_t buildDnsResponse(const uint8_t* query, size_t queryLen,
...
[[nodiscard]] inline std::expected<size_t, DnsError> tryBuildDnsResponse(const uint8_t* query,
                                                                         size_t queryLen,
                                                                         ...
                                                                         uint32_t answerIp)
{
    ...
    size_t written = buildDnsResponse(query, queryLen, response.data(), response.size(), answerIp);
    if (written == 0)
    {
...
[[nodiscard]] inline size_t extractDomainName(const uint8_t* query, size_t queryLen, char* outBuf,
                                              size_t outCap)
{

    if (queryLen < sizeof(DnsHeader) || outCap == 0)
        return 0;
