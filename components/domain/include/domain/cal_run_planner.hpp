...
enum class CalRunAction
{
    Reject,
    CalDose,
    CalSpeed
};
enum class CalRunRejectReason
{
    None,
    InvalidMode,
    BuretteBusy
...
inline CalRunPlan planCalRun(const char* mode, float speedMlMin, uint16_t freqHz, float maxFreqHz,
                             float speedCoeff, bool isBusy) noexcept
{
    if (mode == nullptr)
    {
    ...
    {
        uint16_t calFreq = (freqHz == 0) ? static_cast<uint16_t>(maxFreqHz / 2.0f + 0.5f) : freqHz;
    ...
    float fillSpeed =
        (speedMlMin < MIN_CAL_SPEED_ML_MIN) ? (maxFreqHz / 2.0f) * speedCoeff : speedMlMin;
