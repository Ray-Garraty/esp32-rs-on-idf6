...
[[nodiscard]] inline SpeedCalResult calculateSpeedCalibration(
    const float* frequencies, const float* speeds, size_t count) noexcept {
    ...
    if (std::fabs(denom) < 0.000001f) return result;
