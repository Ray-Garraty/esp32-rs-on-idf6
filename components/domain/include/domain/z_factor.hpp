...
inline constexpr float Z_TEMPS[Z_TABLE_ROWS] = {
    15.0f, 15.5f, 16.0f, 16.5f, 17.0f, 17.5f, 18.0f, 18.5f, 19.0f, 19.5f, 20.0f,
    20.5f, 21.0f, 21.5f, 22.0f, 22.5f, 23.0f, 23.5f, 24.0f, 24.5f, 25.0f, 25.5f,
    26.0f, 26.5f, 27.0f, 27.5f, 28.0f, 28.5f, 29.0f, 29.5f, 30.0f};
...
{
    if (temperature < Z_TEMPS[0])
        temperature = Z_TEMPS[0];
    if (temperature > Z_TEMPS[Z_TABLE_ROWS - 1])
        temperature = Z_TEMPS[Z_TABLE_ROWS - 1];
    if (pressure < Z_PRESSURES[0])
        pressure = Z_PRESSURES[0];
    if (pressure > Z_PRESSURES[Z_TABLE_COLS - 1])
        pressure = Z_PRESSURES[Z_TABLE_COLS - 1];

    size_t ti = 0;
    size_t pi = 0;
    for (size_t i = 0; i < Z_TABLE_ROWS - 1; ++i)
    {
        if (temperature >= Z_TEMPS[i])
            ti = i;
    }
    for (size_t i = 0; i < Z_TABLE_COLS - 1; ++i)
    {
        if (pressure >= Z_PRESSURES[i])
            pi = i;
    }
...
[[nodiscard]] inline float calculateNewStepsPerMl(float currentSpm, float targetVolMl,
                                                  float actualVolMl) noexcept
{
    if (actualVolMl < 0.0001f)
        return currentSpm;
