...
{
    if (cal.speedCoeff < 0.000001f)
        return Hz{cal.minFreqHz};
    uint32_t freq = static_cast<uint32_t>(speedMlMin.value / cal.speedCoeff + 0.5f);
    if (freq < cal.minFreqHz)
        freq = cal.minFreqHz;
    if (freq > cal.maxFreqHz)
        freq = cal.maxFreqHz;
    return Hz{freq};
}
...
[[nodiscard]] inline DosePlan planDose(float volumeMl, float currentVolumeMl,
                                       const CalibrationData& cal) noexcept
{
    DosePlan plan{};
...
struct VolumeTracker
{
    ...
    void onStopDuringFill(float startVolMl, int32_t stepsTaken, const CalibrationData& cal) noexcept
    {
        ...
        if (currentVolumeMl > cal.nominalVolumeMl)
            currentVolumeMl = cal.nominalVolumeMl;
    }
    void onStopDuringEmpty(float startVolMl, int32_t stepsTaken,
                           const CalibrationData& cal) noexcept
    {
        currentVolumeMl = startVolMl - static_cast<float>(stepsTaken) / cal.stepsPerMl;
        if (currentVolumeMl < 0.0f)
