...
{
    esp_err_t err = nvs_open(ns, readWrite ? NVS_READWRITE : NVS_READONLY, &handle_);
    if (err == ESP_OK)
    {
        open_ = true;
    }
...
domain::Result<std::optional<uint8_t>, domain::ResourceError>
NvsHandle::getU8(const char* key) const
{
    uint8_t value = 0;
...
domain::Result<std::optional<uint32_t>, domain::ResourceError>
NvsHandle::getU32(const char* key) const
{
    uint32_t value = 0;
...
domain::Result<std::optional<int32_t>, domain::ResourceError>
NvsHandle::getI32(const char* key) const
{
    int32_t value = 0;
...
domain::Result<std::optional<std::string_view>, domain::ResourceError>
NvsHandle::getStr(const char* key, char* buf, size_t bufSize) const
{
    size_t len = bufSize;
...
domain::Result<void, domain::ResourceError> NvsHandle::setStr(const char* key,
    ...
    // Create calibration namespaces if they don't exist (NVS_READWRITE auto-creates)
    {
        ...
        if (nvs.isValid())
        {
            ...
            if (r && !r->has_value())
            {
                ...
                std::ignore =
                    nvs.setU32(config::NVS_KEY_CAL_MIN_FREQ, static_cast<uint32_t>(cal.minFreqHz));
                std::ignore =
                    nvs.setU32(config::NVS_KEY_CAL_MAX_FREQ, static_cast<uint32_t>(cal.maxFreqHz));
    ...
    }
    {
        ...
        if (nvs.isValid())
        {
            ...
            if (r && !r->has_value())
            {
                ...
                std::ignore =
                    nvs.setI32(config::NVS_KEY_ADC_B, static_cast<int32_t>(config::ADC_DEFAULT_B));
...
domain::Result<domain::CalibrationData, domain::ResourceError> calibrationRead()
{
    ...
    domain::CalibrationData cal{};
    {
        auto r = nvs.getF32(config::NVS_KEY_CAL_SPM);
        cal.stepsPerMl =
            (r && r->has_value()) ? r->value() : domain::CalibrationData::kDefaultStepsPerMl;
    }
    {
        auto r = nvs.getF32(config::NVS_KEY_CAL_NOM);
        cal.nominalVolumeMl =
            (r && r->has_value()) ? r->value() : domain::CalibrationData::kDefaultNominalVolumeMl;
    }
    {
        auto r = nvs.getF32(config::NVS_KEY_CAL_COEFF);
        cal.speedCoeff =
            (r && r->has_value()) ? r->value() : domain::CalibrationData::kDefaultSpeedCoeff;
    }
    {
        auto r = nvs.getU32(config::NVS_KEY_CAL_MIN_FREQ);
        cal.minFreqHz = (r && r->has_value()) ? static_cast<uint16_t>(r->value() & 0xFFFF)
                                              : domain::CalibrationData::kDefaultMinFreqHz;
    }
    {
...
void adcCalibrationRead(uint16_t& aX1000, int16_t& b)
{
    ...
    {
        auto r = nvs.getU32(config::NVS_KEY_ADC_A_X1000);
        aX1000 = (r && r->has_value()) ? static_cast<uint16_t>(r->value() & 0xFFFF)
                                       : config::ADC_DEFAULT_A_X1000;
    }
    {
        auto r = nvs.getI32(config::NVS_KEY_ADC_B);
        b = (r && r->has_value()) ? static_cast<int16_t>(r->value()) : config::ADC_DEFAULT_B;
    }
}
