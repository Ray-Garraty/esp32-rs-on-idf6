...
[[nodiscard]] std::expected<Command, domain::ProtocolError> parseCommand(std::string_view json);

// Serialize a single JSON value into the response buffer
[[nodiscard]] std::expected<size_t, domain::ProtocolError>
serializeToBuffer(const CommandResponse& rsp, domain::memory::ResponseBuffer& buf);
