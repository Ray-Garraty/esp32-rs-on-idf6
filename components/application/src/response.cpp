...
size_t formatSmResult(domain::memory::ResponseBuffer& buf, uint64_t resultId,
                      const domain::SmResult& smResult)
{
    ...
    if (smResult.type == domain::SmResult::Type::None && smResult.stepsTaken > 0)
    {
        ...
        float volDispensed = cal ? static_cast<float>(smResult.stepsTaken) / cal->stepsPerMl : 0.0f;
        off = static_cast<size_t>(std::snprintf(
            buf.data(), buf.size(),
            R"({"id":%llu,"status":"ok","data":{"volume_dispensed_ml":%.2f}})",
            static_cast<unsigned long long>(resultId), static_cast<double>(volDispensed)));
    }
    else if (smResult.type == domain::SmResult::Type::RinseComplete)
    {
        off = static_cast<size_t>(std::snprintf(
            buf.data(), buf.size(), R"({"id":%llu,"status":"ok","data":{"cycles_completed":1}})",
    ...
    {
        off = static_cast<size_t>(std::snprintf(
            buf.data(), buf.size(), R"({"id":%llu,"status":"ok","data":{"speed_ml_min":%.2f}})",
            static_cast<unsigned long long>(resultId),
    ...
    {
        off = static_cast<size_t>(std::snprintf(
            buf.data(), buf.size(), R"({"id":%llu,"status":"ok","data":{"status":"complete"}})",
            static_cast<unsigned long long>(resultId)));
    }
    else if (smResult.type == domain::SmResult::Type::Error)
    {
        off = static_cast<size_t>(std::snprintf(
            buf.data(), buf.size(), R"({"id":%llu,"status":"error","message":"start_failed"})",
            static_cast<unsigned long long>(resultId)));
    }
    else
    {
