...
void StackMonitor::registerByHandle(TaskHandle_t handle, const char* name,
                                    size_t stackSize) noexcept
{
    if (count_ >= MAX_THREADS)
    {
        ESP_LOGW(TAG, "Too many threads registered");
...
{ // NOLINT(readability-function-cognitive-complexity) // reason: aggregates watermarks across all
  // registered tasks
    for (size_t i = 0; i < count_; ++i)
    {
        ...
        uint32_t usedBytes = static_cast<uint32_t>(stackSizes_[i] - static_cast<size_t>(wm));
        uint32_t usedPct =
            (stackSizes_[i] > 0) ? (usedBytes * 100 / static_cast<uint32_t>(stackSizes_[i])) : 0;
        ESP_LOGI(TAG, "Thread %s: cfg=%zuB wmark=%u used=%u%%", names_[i], stackSizes_[i],
                 static_cast<unsigned>(wm), static_cast<unsigned>(usedPct));
        if (usedPct > 90)
        {
            ESP_LOGW(TAG, "Thread %s: LOW STACK! %u%% used", names_[i],
...
void StackMonitor::logAllWatermarks(void (*print)(const char*)) const noexcept
{
    for (size_t i = 0; i < count_; ++i)
    {
        ...
        char buf[128];
        int n = std::snprintf(buf, sizeof(buf), "Thread %s: cfg=%zuB wmark=%u used=%u%%\n",
                              names_[i], stackSizes_[i], static_cast<unsigned>(wm),
                              static_cast<unsigned>(usedPct));
        ...
        {
            char warn[64];
            int wn = std::snprintf(warn, sizeof(warn), "Thread %s: LOW STACK! %u%% used\n",
