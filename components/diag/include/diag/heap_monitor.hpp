...
inline void print_heap_stats()
{
    ...
    ESP_LOGI("heap", "PSRAM free=%lu largest=%lu | DRAM free=%lu largest=%lu",
             (unsigned long)psram_free, (unsigned long)psram_largest, (unsigned long)dram_free,
             (unsigned long)dram_largest);
    ...
    {
        ESP_LOGW("heap", "CRITICAL: PSRAM largest block < 256 KB (%lu)",
